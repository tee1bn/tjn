	...
	function Cart(){
		...
		this.add_item = function($item, $with_offer_id=null){
			...
			$item.qty = 1;
			$item.deal_price = $item.market_details.price;

			if ($with_offer_id != null) {
				$item.offer_id = $with_offer_id;
				$item.deal_price = $item.market_details.offers_available[$item.offer_id].details_array.price;
			}
		...
		this.calculate_total = function (){
			...
			for(x in this.$items){
				...
				$total = $total + (parseInt($item.deal_price) * parseInt($item.qty) );
			}
